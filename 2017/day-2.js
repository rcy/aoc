...
function check(name, result, expected) {
  if (result !== expected) {
    console.error(`fail: ${name}: ${result} did not match ${expected}`);
  }
}

check('testSheet', checksum(testSheet), 18)

console.log('part 1', checksum(sheet))
...
check('testSheet part 2', checksum2(testSheet2), 9)

function divides(a, b) {
  const min = Math.min(a, b)
  const max = Math.max(a, b)
  if (max / min === Math.floor(max / min)) {
    return max / min;
  }
  return 0;
}
  ...
  for (let r = 0; r < sheet.length; r++) {
    ...
    for (let c = 0; c < row.length - 1; c++) {
      for (let d = c + 1; d < row.length; d++) {
        const quotient = divides(row[c], row[d])
        sum += quotient;
      }
    }
  }
...
console.log('part 2', checksum2(sheet))
